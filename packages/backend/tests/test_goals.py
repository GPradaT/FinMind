...
class TestGoalsCRUD:
    ...
    def test_list_goals(self, client, auth_header):
        client.post("/goals/", json={
            "name": "Goal 1", "target_amount": 1000
        }, headers=auth_header)
        client.post("/goals/", json={
            "name": "Goal 2", "target_amount": 2000
        }, headers=auth_header)
    ...
    def test_update_goal(self, client, auth_header):
        r = client.post("/goals/", json={
            "name": "Old Name", "target_amount": 1000
        }, headers=auth_header)
        goal_id = r.get_json()["id"]

        r = client.put(f"/goals/{goal_id}", json={
            "name": "New Name", "target_amount": 2000
        }, headers=auth_header)
        assert r.status_code == 200
    ...
    def test_cancel_goal(self, client, auth_header):
        r = client.post("/goals/", json={
            "name": "Cancel Me", "target_amount": 500
        }, headers=auth_header)
        goal_id = r.get_json()["id"]
...
class TestContributions:
    def test_add_contribution(self, client, auth_header):
        r = client.post("/goals/", json={
            "name": "Fund", "target_amount": 1000
        }, headers=auth_header)
        goal_id = r.get_json()["id"]

        r = client.post(f"/goals/{goal_id}/contribute", json={
            "amount": 250, "note": "First deposit"
        }, headers=auth_header)
        assert r.status_code == 201
    ...
    def test_auto_complete_on_target(self, client, auth_header):
        r = client.post("/goals/", json={
            "name": "Quick Goal", "target_amount": 100
        }, headers=auth_header)
        goal_id = r.get_json()["id"]

        client.post(f"/goals/{goal_id}/contribute", json={"amount": 100}, headers=auth_header)
    ...
    def test_list_contributions(self, client, auth_header):
        ...
        client.post(f"/goals/{goal_id}/contribute", json={"amount": 100}, headers=auth_header)
        client.post(f"/goals/{goal_id}/contribute", json={"amount": 200}, headers=auth_header)
    ...
    def test_contribute_to_cancelled_goal(self, client, auth_header):
        r = client.post("/goals/", json={
            "name": "Dead Goal", "target_amount": 500
        }, headers=auth_header)
        goal_id = r.get_json()["id"]
        client.delete(f"/goals/{goal_id}", headers=auth_header)

        r = client.post(f"/goals/{goal_id}/contribute", json={"amount": 50}, headers=auth_header)
        assert r.status_code == 400
...
class TestProgress:
    def test_progress_endpoint(self, client, auth_header):
        r = client.post("/goals/", json={
            "name": "House", "target_amount": 50000,
            "deadline": (date.today() + timedelta(days=365)).isoformat()
        }, headers=auth_header)
        goal_id = r.get_json()["id"]

        client.post(f"/goals/{goal_id}/contribute", json={"amount": 12500}, headers=auth_header)
